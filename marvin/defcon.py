...
async def defcon_handler(request: Request):
    ...
    try:
        old_level = int(re.compile(r"\d").findall(defcon_pin["message"]["text"])[0])
    except:
        old_level = None

    lower_match = re.compile(r"^lower($|\s)")
    raise_match = re.compile(r"^raise($|\s)")
    level_match = re.compile(r"^\d($|\s)")
